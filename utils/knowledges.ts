import { CoursesType, KnowledgesType } from "@/types/knowledges";

export const knowledgesData: KnowledgesType[] = [
   {
      icon: "/uiuxdesign-icon.svg",
      title: "UI/UX Design",
      text: [
         "Cores, tipografia, espaçamento, grid, hierarquia, prototipação e style guide.",
      ],
   },
   {
      icon: "/monitor-icon.svg",
      title: "Desenvolvimento Web",
      text: [
         "HTML5 para estruturar e dar significado ao conteúdo usando tags semânticas.",
         "CSS3 para aplicação da UI usando FlexBox e CSS Grid.",
         "JavaScript para criar interações, otimizando a experiência e guiando a atenção do usuário.",
      ],
   },
   {
      icon: "/figma-icon.svg",
      title: "Figma",
      text: [
         "Figma como ferramenta principal para criação de interfaces de usuário e prototipagem de design, utilizando boas práticas como componetização e auto layout.",
      ],
   },
   {
      icon: "/git-icon.svg",
      title: "Controle de versionamento",
      text: [
         "Git para controlar e administrar as versões dos projetos e arquivos.",
         "Github para hospedar os projetos e arquivos.",
      ],
   },
];

export const technologiesData = [
   "HTML",
   "CSS",
   "Sass",
   "JavaScript",
   "Typescript",
   "Git",
   "ReactJS",
   "NextJS",
   "Tailwind",
   "Figma",
];

export const coursesData: CoursesType[] = [
   {
      title: "Web Design",
      company: "Origamid",
      hours: "44h",
      link: "https://www.origamid.com/certificate/0c9093e7",
   },
   {
      title: "CSS Flexbox",
      company: "Origamid",
      hours: "6h",
      link: "https://www.origamid.com/certificate/bb39bc75",
   },
   {
      title: "CSS Grid",
      company: "Origamid",
      hours: "10h",
      link: "https://www.origamid.com/certificate/e634a3d6",
   },
   {
      title: "UI Design",
      company: "Origamid",
      hours: "30h",
      link: "https://www.origamid.com/certificate/6ce6a7b1",
   },
   {
      title: "TypeScript",
      company: "Origamid",
      hours: "22h",
      link: "https://www.origamid.com/certificate/37907eb0",
   },
];
